...
import { Box, Button, Container, Grid, Rating, Typography, useMediaQuery } from "@mui/material";
import banner from "../../../assets/vendorDetailBanner.webp";
...
const Vendordetail = () => {
	...
	const fetchVendorSeller = (slug) => {
		handleLoader(true);
		axios.get(`${baseUrl}/mv/api/store/profile/${slug}`, {
			headers: {
			...
			<Wrapper maxWidth={false}>
				...
				{isSmallScreen ? (
					<SmHeader>
						...
						<Box sx={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingInline: '10px' }}>
							<Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '60px', width: '60px', borderRadius: '50%', border: '4px solid #e1e3eb', backgroundColor: "#fff" }}>
								<StorefrontOutlinedIcon sx={{ fontSize: '30px', color: '#009444' }} />
							</Box>
							<Box sx={{ display: 'flex', flexDirection: 'column', justifyContent: 'start', alignItems: 'start' }}>
								<Typography sx={{ fontSize: "14px", fontWeight: "700", color: "#3866DF" }}>
									{vendorSeller?.profile?.company_name}
								</Typography>
								<Typography sx={{ fontSize: "10px", fontWeight: "500", color: "#9BA0B1" }}>
									<span style={{ fontWeight: "700", color: "#404553" }}>76%</span> Positive Ratings
								</Typography>
								{vendorSeller?.ratings_count > 0 ? (
									<Box sx={{ display: 'flex', flexDirection: 'row', justifyContent: 'start', alignItems: 'center', }}>
										<Typography sx={{ fontSize: "14px", fontWeight: "800", color: "#404553" }}>
											{vendorSeller?.ratings_count}
										</Typography>
										<Rating name="read-only" value={vendorSeller?.avg_rating} size="small" readOnly />
									</Box>
							...
							<Box sx={{ display: 'flex', flexDirection: 'column', justifyContent: 'start', alignItems: 'start', }}>
								<VendorSubDetail >
									<LocalPhoneOutlinedIcon sx={{ fontSize: '14px', color: '#009444' }} />
									<SubHeading>{vendorSeller?.profile?.phone_number}</SubHeading>
								</VendorSubDetail>
								<VendorSubDetail >
									<EmailOutlinedIcon sx={{ fontSize: '14px', color: '#009444' }} />
									<SubHeading>{vendorSeller?.profile?.email}</SubHeading>
								</VendorSubDetail>
								{vendorSeller?.profile?.company_address ? (
									<VendorSubDetail >
										<LocationOnOutlinedIcon sx={{ fontSize: '14px', color: '#009444' }} />
										<SubHeading>{vendorSeller?.profile?.company_address}</SubHeading>
									</VendorSubDetail>
						...
						<Box sx={{ display: 'flex', flexDirection: 'row', justifyContent: 'center', alignItems: 'center' }}>
							<VendorSubDetail sx={{ width: '90%' }}>
								<QuoteBtn onClick={() => vendorSeller?.profile?.website} >More Info <ArrowOutwardIcon />  </QuoteBtn>
								<QuoteBtn>Share <ShareOutlinedIcon />  </QuoteBtn>
							</VendorSubDetail>
						</Box>
					</SmHeader>
				) : (
					<Header>
						<BannerCard dir={getDirection()}>
							...
							<VendorDetail dir={getDirection()}>
								<Typography sx={{ fontSize: "16px", fontWeight: "800", color: "#3866DF" }}>
									{vendorSeller?.profile?.company_name}
								</Typography>
								<Typography sx={{ fontSize: "14px", fontWeight: "600", color: "#9BA0B1" }}>
									<span style={{ fontWeight: "800", color: "#404553" }}>76%</span> Positive Ratings
								</Typography>
							</VendorDetail>
							{vendorSeller?.ratings_count > 0 ? (
								<VendorDetail>
									<Box sx={{ display: 'flex', flexDirection: 'row', justifyContent: 'start', alignItems: 'center', }}>
										<Typography sx={{ fontSize: "26px", fontWeight: "bold", color: "#404553" }}>
											{vendorSeller?.ratings_count}
										</Typography>
										<Rating name="read-only" value={vendorSeller?.avg_rating} readOnly />
									</Box>
									<SubHeading>
							...
							<VendorDetail sx={{ paddingRight: '5px', gap: '10px' }} dir={getDirection()}>
								<VendorSubDetail>
									<LocalPhoneOutlinedIcon sx={{ fontSize: '22px', color: '#009444' }} />
									<SubHeading>{vendorSeller?.profile?.phone_number}</SubHeading>
								</VendorSubDetail>
								<VendorSubDetail>
									<EmailOutlinedIcon sx={{ fontSize: '22px', color: '#009444' }} />
									<SubHeading>{vendorSeller?.profile?.email}</SubHeading>
								</VendorSubDetail>
							</VendorDetail>
							{vendorSeller?.profile?.company_address ? (
								<VendorSubDetail>
									<LocationOnOutlinedIcon sx={{ fontSize: '22px', color: '#009444' }} />
									<SubHeading>{vendorSeller?.profile?.company_address}</SubHeading>
								</VendorSubDetail>
							) : (
								""
				...
				<VendorWrapper container my={5} dir={getDirection()}>
					<FiltersWrapper item lg={3} md={4} sm={4} xs={12} >
						<FilterBox my={3} dir={getDirection()}>
							<Box variant="contained" onClick={handleCategoryToggle} sx={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', cursor: 'pointer', }} >
								<Typography sx={{ fontSize: '16px', fontWeight: '700' }}>{translate("productlisting.cat")}</Typography>
								<Button >
							...
							{catExpand && (
								<FormControl component="fieldset">
									<RadioGroup aria-label="category" value={selectedCategory} onChange={handleCategoryChange}>
										{vendorSeller?.categories.map((category, index) => (
											<FormControlLabel key={index} value={category.name} control={<Radio />}
												label={
													language === "ar"
														? category.arb_name
														: category.name
                  ...
                  <RadioGroup aria-label="category" value={selectedSubcategory} onChange={handleSubcategoryChange}>
                    {vendorSeller?.subcategories.map((subcategory, index) => (
                      <FormControlLabel key={index} value={subcategory.name} control={<Radio />}
						...
						<FilterBox my={3} dir={getDirection()}>
							<Box variant="contained" onClick={handlePriceToggle} sx={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', cursor: 'pointer', }} >
								<Typography sx={{ fontSize: '16px', fontWeight: '700' }}>{translate("productlisting.range")}</Typography>
								<Button >
							...
							{priceExpand && (
								<FormControl component="fieldset">
									<RadioGroup aria-label="category" value={selectedPrice} onChange={handlePriceChange}>
						...
						<FilterBox my={3} dir={getDirection()}>
							<Box variant="contained" onClick={handleBrandToggle} sx={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', cursor: 'pointer', }} >
								<Typography sx={{ fontSize: '16px', fontWeight: '700' }} >{translate("productlisting.brand")}</Typography>
								<Button >
							...
							{brandExpand && (
								<FormControl component="fieldset">
									<RadioGroup aria-label="category" value={selectedBrand} onChange={handleBrandChange}>
										{vendorSeller?.brands.map((brands, index) => (
											<FormControlLabel key={index} value={brands.name} control={<Radio />}
												label={
													language === "ar"
														? brands.arb_name
														: brands.name} />
...
const SmHeader = styled(Box)(({ theme }) => ({
	width: "100%",
	display: "flex",
	flexDirection: "column",
	height: 'auto',
...
const BannerCard = styled(Container)(({ theme }) => ({
	width: '100%',
	padding: '20px 10px',
	marginRight: '0',
	display: "flex",
...
const VendorDetail = styled(Box)(({ theme }) => ({
	display: "flex",
	flexDirection: "column",
	alignItems: "start",
	justifyContent: 'center',
...
const VendorSubDetail = styled(Box)(({ theme }) => ({
	display: "flex",
	flexDirection: "row",
	alignItems: "center",
	justifyContent: 'center',
